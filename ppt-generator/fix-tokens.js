...
constants.forEach(c => {
  ...
  // Need to be careful not to replace T.CONSTANT with T.T.CONSTANT if run twice, 
  // but this is a one-time script.
  content = content.replace(usageRegex, `T.${c}`);
});
...
content = content.replace(/grad\.addColorStop\(([^,]+),\s*'rgba\([^)]+\)'\);/g, (match) => {
  if (match.includes('${hexToRgba')) {
    // Already converted, let's wrap it properly so it evaluates
