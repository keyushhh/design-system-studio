...
function resolveColorMixForHtml2Canvas(clonedDoc: Document) {
  // Sync the CSS variables and theme applied dynamically by SyncEngine
  if (typeof document !== 'undefined' && document.documentElement) {
    clonedDoc.documentElement.style.cssText = document.documentElement.style.cssText;
    const theme = document.documentElement.getAttribute('data-theme');
    if (theme) clonedDoc.documentElement.setAttribute('data-theme', theme);
  }
  ...
  try {
    return await html2canvas(el, {
      ...
      onclone: (clonedDoc: Document) => {
        resolveColorMixForHtml2Canvas(clonedDoc);
        return new Promise<void>((resolve) => {
          // Give the iframe time to load Google Fonts before capturing.
          // Wait for the fonts API if available in the iframe, or fallback to a timeout.
          const win = clonedDoc.defaultView;
          if (win && 'fonts' in win.document) {
            (win.document as any).fonts.ready.then(() => {
              setTimeout(resolve, 150); // slight extra buffer for paint
            });
          } else {
