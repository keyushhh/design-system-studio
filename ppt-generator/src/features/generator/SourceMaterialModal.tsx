...
export function SourceMaterialModal({ open, onClose, onDocumentParsed, onImport, onLoadLibraryEntry, hasSource }: SourceMaterialModalProps) {
  ...
  return (
    ...
    >
      <div
        ref={panelRef}
        className="flex flex-col w-full max-w-2xl max-h-[86vh] bg-[var(--pure-white)] rounded-[var(--radius-sharp)] shadow-2xl overflow-hidden"
                  ...
                  {(['All', ...CAMPAIGN_TYPES] as const).map((c) => (
                    <button
                      ...
                      className={`h-[28px] px-3 text-[11.5px] font-bold rounded-[var(--radius-sharp)] border transition-colors cursor-pointer ${
                        campaignFilter === c
                          ? 'bg-neutral-900 border-neutral-900 text-[var(--pure-white)]'
                          : 'bg-[var(--pure-white)] border-neutral-200 text-neutral-600 hover:border-neutral-400'
                      }`}
                    >
                      {c}
                    </button>
          ...
          {tab === 'prompt' && (
            <div className="flex flex-col gap-3">
              ...
              <div className="flex items-center gap-2.5">
                <button
                  onClick={handleCopy}
                  className={`h-[40px] px-5 flex items-center gap-2 text-[13px] font-bold text-[var(--pure-white)] rounded-[var(--radius-sharp)] transition-colors cursor-pointer ${
                ...
                <button
                  ...
                  className="h-[40px] px-4 flex items-center gap-1.5 text-[13px] font-semibold text-neutral-700 bg-[var(--pure-white)] hover:bg-neutral-50 border border-neutral-200 rounded-[var(--radius-sharp)] transition-colors cursor-pointer"
                >
          ...
          {tab === 'paste' && (
            <div className="flex flex-col gap-3">
              ...
              <button
                ...
                className="self-start h-[40px] px-5 text-[13px] font-bold text-[var(--pure-white)] bg-neutral-900 hover:bg-neutral-800 rounded-[var(--radius-sharp)] transition-colors cursor-pointer disabled:opacity-40 disabled:cursor-not-allowed"
              >
