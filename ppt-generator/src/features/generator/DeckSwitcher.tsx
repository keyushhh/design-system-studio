...
export function DeckSwitcher({ projects, activeId, onSwitch, onNew, onRename, onDelete }: DeckSwitcherProps) {
  ...
  return (
    <div ref={rootRef} className="relative w-full">
      <button
        onClick={() => setOpen((v) => !v)}
        className="w-full flex items-center justify-between gap-2 h-[36px] px-3 rounded-[var(--radius-sharp)] border border-neutral-200 bg-[var(--pure-white)] hover:bg-neutral-50 transition-colors cursor-pointer text-left"
      >
      ...
      {open && (
        <div className="absolute left-0 right-0 top-[calc(100%+4px)] z-40 w-full bg-[var(--pure-white)] border border-neutral-200 rounded-[var(--radius-sharp)] shadow-lg overflow-hidden">
