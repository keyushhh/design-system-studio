...
const PLACEHOLDER =
  'Placeholder content for the Design System Studio Master Template. This section will automatically populate once a Document is provided.';
...
function drawGlow(ctx: CanvasRenderingContext2D, g: GlowSpec, isDark?: boolean) {
  ...
  if (isDark) {
    grad.addColorStop(0, hexToRgba(T.EMERALD_500, 0.35));
    grad.addColorStop(0.45, hexToRgba(T.EMERALD_500, 0.14));
    grad.addColorStop(1, hexToRgba(T.EMERALD_500, 0));
  } else {
    grad.addColorStop(0, hexToRgba(T.EMERALD_500, 0.08));
    grad.addColorStop(0.7, hexToRgba(T.EMERALD_500, 0));
  }
  ctx.save();
  ctx.fillStyle = grad;
...
const DECOR: Record<string, DecorConfig> = {
  s1: { base: T.WHITE, grid: true, glow: { cx: 1520, cy: 400, r: 700 } },
  s2: { base: T.WHITE, grid: true },
  s3: { base: T.WHITE, grid: true },
  s4: { base: T.BLACK },
  s5: { base: T.WHITE, grid: true },
  s6: { base: T.WHITE, grid: true, glow: { cx: 960, cy: 540, r: 700 } },
  s7: { base: T.WHITE, grid: true },
  s8: { base: T.WHITE, grid: true },
  s9: { base: T.WHITE, grid: true },
  s10: { base: T.WHITE, grid: true },
  s11: { base: T.WHITE, grid: true },
  s12: { base: T.WHITE, grid: true },
  s13: { base: T.WHITE, grid: true, glow: { cx: 200, cy: 880, r: 700 } },
  s14: { base: T.BLACK },
};
...
function addText(
  ...
  slide.addText(runs, {
    ...b,
    fontFace: o.fontFace ?? T.FONT_DISPLAY,
    fontSize: pt(o.size),
    bold: o.bold,
    italic: o.italic,
    color: o.color ?? T.NEUTRAL_900,
    align: o.align ?? 'left',
    valign: o.valign ?? 'top',
...
function addHudTop(slide: pptxgen.Slide, label: string, num: string, isDark?: boolean) {
  addText(slide, label.toUpperCase(), box(80, 55, 800, 30), {
    ...
    size: 12,
    color: isDark ? T.NEUTRAL_400 : T.NEUTRAL_500,
  ...
  addLine(slide, 80, 92, 1840, 92, isDark ? '27272A' : T.NEUTRAL_200, 1);
}
...
function addEditorialLabel(slide: pptxgen.Slide, text: string, xPx: number, yPx: number, opts?: { size?: number; center?: boolean; color?: string }) {
  ...
  const color = opts?.color ?? T.EMERALD_600;
  if (opts?.center) {
    ...
    addLine(slide, xPx, yPx + size / 2 + 3, xPx + 40, yPx + size / 2 + 3, T.EMERALD_500, 1);
...
async function buildCover(slide: pptxgen.Slide, content: SlideInstance['content'], logoUrl?: string, isDark = false) {
  ...
  const runs: pptxgen.TextProps[] = lines.map((line, i) => ({
    ...
    options: {
      ...
      ...(i === lines.length - 1 && lines.length > 1 ? { color: isDark ? '475569' : T.NEUTRAL_300 } : { color: isDark ? T.WHITE : T.NEUTRAL_900 }),
    },
  }));
  ...
  addLine(slide, 140, taglineY + 9, 275, taglineY + 9, T.EMERALD_500, 1);
  addText(slide, content.tagline ?? PLACEHOLDER, box(323, taglineY - 10, 1460, 40), {
    fontFace: T.FONT_MONO,
    size: 18,
    color: isDark ? '94A3B8' : T.NEUTRAL_500,
    letterSpacingEm: 0.25,
  });
  ...
  addText(slide, 'PROPRIETARY AND CONFIDENTIAL', box(80, 1000, 500, 26), {
    fontFace: T.FONT_MONO,
    size: 11,
    color: isDark ? '64748B' : T.NEUTRAL_400,
  });
...
function buildIndex(slide: pptxgen.Slide, content: SlideInstance['content'], num: string, isDark = false) {
  ...
  addText(slide, heading, box(140, 215, headingW, headingH), {
    size: headingFont,
    bold: true,
    color: isDark ? T.WHITE : T.NEUTRAL_900,
    lineSpacingMultiple: 0.9,
  });
  ...
  parts.slice(0, 4).forEach((part, i) => {
    ...
    addLine(slide, cx, cy, cx, cy + rowH, i === 0 ? T.EMERALD_500 : isDark ? '27272A' : T.NEUTRAL_200, 2);
    addEditorialLabel(slide, `Part 0${i + 1}`, cx + 30, cy, { size: 10 });
    addText(slide, part.title, box(cx + 30, cy + 30, colW - 30, 50), { size: 32, bold: true, color: isDark ? T.WHITE : T.NEUTRAL_900, lineSpacingMultiple: 1.05 });
...
function buildExecutiveSummary(slide: pptxgen.Slide, content: SlideInstance['content'], num: string, isDark = false) {
  ...
  addText(slide, content.body ?? PLACEHOLDER, box(140, bodyY, leftW, 400), {
    fontFace: T.FONT_DISPLAY,
    size: 32,
    color: isDark ? 'CBD5E1' : T.NEUTRAL_500,
    lineSpacingMultiple: 1.5,
  });

  const rightPadLeft = 66;
  addLine(slide, rightX, bodyY, rightX, bodyY + 340, isDark ? '27272A' : T.NEUTRAL_200, 1);
...
async function buildSectionDivider(slide: pptxgen.Slide, content: SlideInstance['content'], num: string, logoUrl?: string) {
  ...
  const lines = estimateWrappedLines(headingText, 180, 1620);
  const headingHeight = lines * 180 * 1.1; // estimate line height
  const subtitleY = 463 + headingHeight + 40;

  addEditorialLabel(slide, content.eyebrow ?? 'Part 02', 150, 400, { color: T.EMERALD_400 });
  addText(slide, headingText, box(150, 463, 1620, Math.max(260, headingHeight)), {
    size: 180,
    bold: true,
    color: T.WHITE,
  });
  addText(slide, content.subtitle ?? PLACEHOLDER, box(150, subtitleY, 960, 150), {
    fontFace: T.FONT_DISPLAY,
    size: 30,
    color: T.WHITE,
    transparency: 45,
...
function buildTwoColumnContext(slide: pptxgen.Slide, content: SlideInstance['content'], num: string, isDark = false) {
  ...
  addLine(slide, 960, 0, 960, 1080, isDark ? '27272A' : T.NEUTRAL_200, 1);
  ...
  addText(slide, attrRuns, box(140, 690, 700, 180), {
    fontFace: T.FONT_MONO,
    size: 20,
    color: isDark ? '94A3B8' : T.NEUTRAL_400,
    lineSpacingMultiple: 1.6,
  });

  addRect(slide, box(960, 0, 960, 1080), isDark ? '121215' : T.NEUTRAL_50);
  ...
  addText(slide, content.rightBody ?? PLACEHOLDER, box(1000, 515, 780, 300), {
    fontFace: T.FONT_DISPLAY,
    size: 32,
    color: isDark ? 'CBD5E1' : T.NEUTRAL_900,
    lineSpacingMultiple: 1.5,
  });
}
...
function buildDataMonument(slide: pptxgen.Slide, content: SlideInstance['content'], isDark = false) {
  ...
  const runs: pptxgen.TextProps[] = [
    { text: content.value ?? '000.0', options: { color: isDark ? T.WHITE : T.NEUTRAL_900 } },
    { text: ` ${content.unit ?? 'M'}`, options: { color: T.EMERALD_500, fontSize: pt(420 * 0.3) } },
  ];
...
const NEUTRAL_RAMP_HEX = [T.NEUTRAL_300, T.NEUTRAL_400, T.NEUTRAL_200];
function metricColorHex(bar: { active?: boolean }, i: number): string {
  return bar.active ? T.EMERALD_500 : NEUTRAL_RAMP_HEX[i % NEUTRAL_RAMP_HEX.length];
}
...
function buildMetricsChart(slide: pptxgen.Slide, bars: SlideInstance['content']['bars'], chartType: string, b: Box) {
  ...
  const shared: pptxgen.IChartOpts = {
    ...b,
    showValue: true,
    dataLabelFontFace: T.FONT_MONO,
    dataLabelFontSize: pt(14),
    dataLabelColor: T.NEUTRAL_500,
    dataLabelFormatCode: '0"%"',
    catAxisLabelFontFace: T.FONT_MONO,
    catAxisLabelFontSize: pt(13),
    catAxisLabelColor: T.NEUTRAL_500,
    catAxisLineColor: T.NEUTRAL_300,
    valAxisHidden: true,
    showLegend: false,
    chartColors: [T.EMERALD_500],
  };
  ...
  if (chartType === 'donut') {
    slide.addChart('doughnut', [{ name: 'Series 1', labels, values }], {
      ...
      showLegend: true,
      legendPos: 'r',
      legendColor: T.NEUTRAL_500,
      legendFontFace: T.FONT_MONO,
  ...
  if (chartType === 'line') {
    slide.addChart('line', [{ name: 'Series 1', labels, values }], {
      ...
      lineDataSymbolLineColor: T.EMERALD_500,
      dataLabelPosition: 't',
    });
    return;
  }
...
function buildMetricsDashboard(slide: pptxgen.Slide, content: SlideInstance['content'], num: string, isDark = false) {
  ...
  kpis.slice(0, 3).forEach((k, i) => {
    ...
    addText(slide, k.value, box(x, kpiY + 40, kpiColW, 90), { size: 64, bold: true, color: isDark ? T.WHITE : T.NEUTRAL_900, lineSpacingMultiple: 0.95 });
  });
}
...
function buildComparativeTable(slide: pptxgen.Slide, content: SlideInstance['content'], num: string, isDark = false) {
  ...
  const headerRow: pptxgen.TableRow = headers.map((h) => ({
    ...
    options: {
      ...
      border: [{ type: 'none' }, { type: 'none' }, { type: 'solid', color: isDark ? T.WHITE : T.NEUTRAL_900, pt: 1.5 }, { type: 'none' }],
      valign: 'bottom',
    },
  }));
  ...
  const bodyRows: pptxgen.TableRow[] = rows.map((r) => [
    ...
    { text: r.cur, options: {} },
    { text: r.tgt, options: {} },
    { text: r.delta, options: { color: T.EMERALD_600 } },
  ].map((c) => ({
    text: c.text,
    options: {
      fontFace: T.FONT_DISPLAY,
      fontSize: pt(cellFont),
      color: c.options.color ?? (isDark ? T.WHITE : T.NEUTRAL_900),
      border: [{ type: 'none' }, { type: 'none' }, { type: 'solid', color: isDark ? '27272A' : T.NEUTRAL_200, pt: 0.75 }, { type: 'none' }],
      valign: 'top',
    },
  })));
...
function buildStrategicRoadmap(slide: pptxgen.Slide, content: SlideInstance['content'], num: string, isDark = false) {
  ...
  addText(slide, content.heading ?? 'Pathway to Execution.', box(140, 315, 1640, 100), { size: 100, bold: true, color: isDark ? T.WHITE : T.NEUTRAL_900 });

  const railY = 490;
  addLine(slide, 140, railY, 1780, railY, isDark ? '27272A' : T.NEUTRAL_200, 2);
  ...
  phases.forEach((p, i) => {
    ...
    addCircle(slide, box(x, railY - 10, 20, 20), p.completed ? T.EMERALD_500 : isDark ? '475569' : T.NEUTRAL_300);
    addEditorialLabel(slide, `Phase ${String(i + 1).padStart(2, '0')}`, x, railY + 42, { size: 12 });
    addText(slide, p.title, box(x, railY + 85, itemW, 60), { size: 32, bold: true, color: isDark ? T.WHITE : T.NEUTRAL_900, lineSpacingMultiple: 1.05 });
...
async function buildImageEditorial(slide: pptxgen.Slide, content: SlideInstance['content'], isDark = false) {
  ...
  if (content.imageUrl) {
    ...
    addRect(slide, imgBox, isDark ? '121215' : T.NEUTRAL_100);
...
function buildProcessArchitecture(slide: pptxgen.Slide, content: SlideInstance['content'], num: string, isDark = false) {
  ...
  addText(slide, content.heading ?? 'Operational Flow.', box(140, 315, 1640, 100), { size: 100, bold: true, color: isDark ? T.WHITE : T.NEUTRAL_900 });
  ...
  steps.forEach((s, i) => {
    ...
    const h = 420 - yOffset;
    addRect(slide, box(x, top, colW, h), isDark ? '121215' : undefined, { color: i === 1 ? T.EMERALD_500 : isDark ? '27272A' : T.NEUTRAL_200, widthPx: 1 });
    addText(slide, String(i + 1).padStart(2, '0'), box(x + 40, top + 30, colW - 80, 70), {
      fontFace: T.FONT_MONO,
      size: 48,
      color: T.EMERALD_500,
    });
    addText(slide, s.title, box(x + 40, top + 110, colW - 80, 60), { size: 32, bold: true, color: isDark ? T.WHITE : T.NEUTRAL_900, lineSpacingMultiple: 1.05 });
...
async function buildGlobalMap(slide: pptxgen.Slide, content: SlideInstance['content'], num: string, isDark = false) {
  ...
  addText(slide, content.heading ?? 'Regional Impact.', box(140, 160, leftW, 100), { size: 100, bold: true, color: isDark ? T.WHITE : T.NEUTRAL_900 });
  ...
  if (showMap) {
    ...
    if (content.imageUrl) {
      ...
      addRect(slide, mapBox, undefined, { color: isDark ? '27272A' : T.NEUTRAL_200, widthPx: 1 });
    } else {
      addRect(slide, mapBox, isDark ? '121215' : T.NEUTRAL_50, { color: isDark ? '27272A' : T.NEUTRAL_200, widthPx: 1 });
  ...
  sectors.forEach((s, i) => {
    ...
    addLine(slide, rightX, y, rightX + rightW, y, isDark ? '27272A' : T.NEUTRAL_200, 1);
    addEditorialLabel(slide, s.label, rightX, y + 30, { size: 10 });
    addText(slide, s.value, box(rightX, y + 68, rightW, 90), { size: 72, bold: true, color: isDark ? T.WHITE : T.NEUTRAL_900 });
  });
  void num;
}
...
async function buildFeaturedQuote(slide: pptxgen.Slide, content: SlideInstance['content'], num: string, isDark = false) {
  ...
  if (content.avatarUrl) {
    ...
    addCircle(slide, box(195, avatarY, 84, 84), isDark ? '27272A' : T.NEUTRAL_200);
  }
  addText(slide, content.author ?? 'Author Name', box(304, avatarY + 6, 700, 50), { size: 27, bold: true, color: isDark ? T.WHITE : T.NEUTRAL_900 });
...
async function buildExit(slide: pptxgen.Slide, content: SlideInstance['content'], logoUrl?: string) {
  await addLogo(slide, logoUrl, 80, 60, true);
  addEditorialLabel(slide, content.eyebrow ?? 'Conclusion', 140, 360, { color: T.EMERALD_400 });
  addText(slide, content.heading ?? 'Thank You.', box(140, 415, 1600, 280), { size: 180, bold: true, color: T.WHITE });
  ...
  addText(slide, runs, box(140, 920, 1600, 40), { fontFace: T.FONT_MONO, size: 16, color: T.EMERALD_400 });
}
...
async function buildBlank(slide: pptxgen.Slide, content: SlideInstance['content'], num: string, isDark = false) {
  ...
  if (layout === 'full-bleed') {
    if (content.imageUrl) {
      ...
      addRect(slide, box(0, 0, 1920, 1080), isDark ? '09090B' : T.NEUTRAL_100);
    }
    addRect(slide, box(0, 540, 1920, 540), T.BLACK, undefined, 30);
    ...
    addText(slide, num, box(1040, 55, 800, 30), { fontFace: T.FONT_MONO, size: 12, color: T.WHITE, align: 'right' });
    addEditorialLabel(slide, content.eyebrow ?? 'Section', 140, 780, { color: T.EMERALD_400 });
    addText(slide, content.heading ?? 'Blank Slide.', box(140, 835, 1600, 110), { size: 72, bold: true, color: T.WHITE });
  ...
  if (layout === 'two-column') {
    ...
    addText(slide, content.heading ?? 'Blank Slide.', box(140, 255, 780, 180), { size: 64, bold: true, color: isDark ? T.WHITE : T.NEUTRAL_900, lineSpacingMultiple: 0.95 });
  ...
  addHudTop(slide, content.hudLabel ?? 'Custom Slide', num, isDark);
  addEditorialLabel(slide, content.eyebrow ?? 'Section', 140, 200);
  addText(slide, content.heading ?? 'Blank Slide.', box(140, 255, 1640, 130), { size: 88, bold: true, color: isDark ? T.WHITE : T.NEUTRAL_900 });
...
export async function addNativeSlide(
  ...
  if (instance.templateId === 'blank') {
    ...
    applyBackground(
      slide,
      layout === 'full-bleed' ? { base: isDark ? T.BLACK : T.WHITE } : { base: isDark ? T.BLACK : T.WHITE, grid: true },
      isDark
    );
  } else {
    const defaultDecor = DECOR[instance.templateId] ?? { base: T.WHITE };
    const baseColor = isDark ? T.BLACK : defaultDecor.base;
